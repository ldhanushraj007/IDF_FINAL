...
const BASE_POPULAR_FILTERS: { id: FilterType; label: string; group?: string; isCategory?: boolean }[] = [
...
export default function ShopPage() {
  ...
  const categoryFilters = useMemo(() => [
    ...BASE_POPULAR_FILTERS,
    ...(activeCategories || []).map(c => ({ id: c.slug as FilterType, label: c.name, group: 'Category', isCategory: true })),
  ], [activeCategories]);
  ...
  // Sync filter when URL parameter changes (e.g. /shop/contemporary -> /shop/bridal)
  useEffect(() => {
    setFilter(urlCategory || 'all');
  }, [urlCategory]);
  ...
  const items = useMemo(() => {
    ...
    if (q) {
      ...
      const isTag = BASE_POPULAR_FILTERS.some(f => f.id === filter && f.group === 'Popular');
      if (isTag) {
        list = catalog.filter(i => i.tags.includes(filter as Tag));
      } else {
        const target = String(filter).toLowerCase();
        list = catalog.filter(i =>
          (i.categoryId?.toLowerCase() || '') === target ||
          (i.category?.toLowerCase() || '') === target
        );
      }
    }
  ...
  const activeLabel = categoryFilters.find(f => f.id === filter)?.label ?? 'All Fabrics';
            ...
            <nav className="flex flex-col space-y-2">
              <button
                ...
                className={`text-left font-sans text-[12px] transition-colors ${
                  filter === 'all' ? 'font-bold text-[#1F1916]' : 'text-[#1F1916]/60 hover:text-[#1F1916]'
                }`}
              >
                All Fabrics
              </button>
              {(activeCategories || []).map((cat) => {
                const active = filter === cat.slug || filter === cat.id;
                return (
                  <button
                    key={cat.slug}
                    type="button"
                ...
                {catOpen && (
                  ...
                  >
                    {categoryFilters.map((f, i) => {
                      ...
                      const prevGroup = i > 0 ? categoryFilters[i - 1].group : undefined;
