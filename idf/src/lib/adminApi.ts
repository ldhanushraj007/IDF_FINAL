...
export async function fetchCategories(): Promise<CategoryConfig[]> {
  try {
    ...
    if (data && data.length > 0) {
      localStorage.setItem('idf_categories_cache', JSON.stringify(data));
      return data;
    }
  } catch (err) {}
  try {
...
export async function saveCategories(categories: CategoryConfig[]): Promise<void> {
  localStorage.setItem('idf_categories_cache', JSON.stringify(categories));
  window.dispatchEvent(new Event('idf_categories_updated'));
  if (isAdminConfigured) {
    try {
      await adminPost('save_categories', { categories });
    } catch (err) {
      console.warn('saveCategories backend write failed, saved locally', err);
    }
  }
}
...
export const DEFAULT_COMBOS: ComboDeal[] = [
  {
    id: 'royal-wedding-duo',
    title: 'Royal Wedding Duo (Tulle + Pearl Organza)',
    description: 'When Aurelia Hand-Embroidered Tulle and Noor Pearl Organza are ordered together, receive a 10% bundle discount.',
    productIds: ['aurelia-tulle', 'noor-organza'],
    discountPercent: 10,
    active: true,
  },
];

export async function fetchCombos(): Promise<ComboDeal[]> {
  try {
    const data = await adminPost<ComboDeal[]>('get_combos');
    if (data && data.length > 0) {
      localStorage.setItem('idf_combos_cache', JSON.stringify(data));
      return data;
    }
  } catch (err) {}
  try {
...
export async function saveCombos(combos: ComboDeal[]): Promise<void> {
  localStorage.setItem('idf_combos_cache', JSON.stringify(combos));
  window.dispatchEvent(new Event('idf_combos_updated'));
  if (isAdminConfigured) {
    try {
      await adminPost('save_combos', { combos });
    } catch (e) {
      console.warn('saveCombos backend write failed, saved locally', e);
    }
  }
}
  ...
  try {
    ...
    if (data) {
      ...
      if (data.combos?.length) {
        localStorage.setItem('idf_combos_cache', JSON.stringify(data.combos));
      }
      return data;
    }
  } catch (err) {
    console.warn('Admin bootstrap endpoint skipped; falling back', err);
  }
  return null;
}
