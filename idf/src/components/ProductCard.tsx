...
export default function ProductCard({ item, onQuickView }: ProductCardProps) {
  ...
  const primaryImg = gallery[0] || item.image || '/images/fabrics/f01.jpg';
  const secondaryImg = gallery.length > 1 ? gallery[1] : null;
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#F5F0EB]">
        <Link to={`/shop/product/${item.id}`} className="block w-full h-full relative" tabIndex={soldOut ? -1 : 0}>
          <img
            src={primaryImg}
            alt={item.name}
            loading="lazy"
            ...
            className={`w-full h-full object-cover transition-all duration-500 ease-out group-hover:scale-105 ${
              secondaryImg && hovered ? 'opacity-0' : 'opacity-100'
            }`}
          />
          {secondaryImg && (
            <img
        ...
        {gallery.length > 1 && (
          <span className="absolute bottom-2.5 right-2.5 px-1.5 py-0.5 bg-black/50 backdrop-blur-xs text-white text-[8px] font-sans font-medium tracking-wider rounded pointer-events-none group-hover:opacity-0 transition-opacity">
