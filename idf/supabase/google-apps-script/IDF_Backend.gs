...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      // Combos / Bundle Deals
      case 'get_combos':
        result = getCombosBackend(body);
        break;
      case 'save_combos':
        result = withScriptLock(function() { return saveCombosBackend(body); });
        break;

      // Admin High-Performance Single-Trip Bootstrap
      case 'admin_bootstrap':
        result = getAdminBootstrapBackend(body);
        break;
...
function sheet(name) {
  ...
  if (!sh) {
    ...
    var headers = {
      ...
      Combos:    ['id','title','description','product_ids','discount_percent','active','created_at'],
...
function getCombosBackend(body) {
  var sh = sheet('Combos');
  var data = sh.getDataRange().getValues();
  if (data.length < 2) return { ok: true, data: [] };
  var list = [];
  for (var i = 1; i < data.length; i++) {
    var r = data[i];
    if (!r[0] && !r[1]) continue;
    var pids = String(r[3] || '').split(/[|,]/).map(function(s){ return s.trim(); }).filter(Boolean);
    list.push({
      id: String(r[0]),
      title: String(r[1]),
      description: String(r[2] || ''),
      productIds: pids,
      discountPercent: Number(r[4]) || 10,
...
function saveCombosBackend(body) {
  ...
  for (var i = 0; i < combos.length; i++) {
    ...
    rows.push([
      ...
      c.title || 'Combo Deal',
      c.description || '',
      Array.isArray(c.productIds) ? c.productIds.join('|') : String(c.productIds || ''),
      Number(c.discountPercent) || 10,
      c.active === false ? 'FALSE' : 'TRUE',
...
function getAdminBootstrapBackend(body) {
  var catRes = getCatalog(body);
  var revRes = fetchAllReviewsBackend(body);
  var ordRes = fetchOrdersBackend(body);
  var catsRes = getCategoriesBackend(body);
  var combosRes = getCombosBackend(body);
  var setRes = getSettingsBackend(body);
  return {
    ok: true,
    data: {
      items: (catRes && catRes.data && catRes.data.items) ? catRes.data.items : [],
      offer: (catRes && catRes.data && catRes.data.offer) ? catRes.data.offer : null,
      reviews: (revRes && revRes.data) ? revRes.data : [],
      orders: (ordRes && ordRes.data) ? ordRes.data : [],
      categories: (catsRes && catsRes.data) ? catsRes.data : [],
      combos: (combosRes && combosRes.data) ? combosRes.data : [],
      settings: (setRes && setRes.data) ? setRes.data : {}
    }
  };
}
